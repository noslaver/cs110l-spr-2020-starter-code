...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for (i, x) in seq1.iter().enumerate() {
        for (j, y) in seq2.iter().enumerate() {
            if x == y {
                grid.set(i + 1, j + 1, grid.get(i, j).unwrap() + 1).unwrap();
            } else {
                let val = std::cmp::max(grid.get(i + 1, j), grid.get(i, j + 1));
                grid.set(i + 1, j + 1, val.unwrap()).unwrap();
            }
        }
    }
...
fn main() {
    ...
    let lines1 = read_file_lines(&filename1).expect("Unknown file");
    let lines2 = read_file_lines(&filename2).expect("Unknown file");
